...
description: Delegate coding tasks to user-configured MCPishCode subagents.
---
...
mcpishcode agents run <profile-or-provider-or-id> "<prompt>"
mcpishcode agents show <id>
```

`ls` shows existing subagent sessions for the current workspace. MCPishCode scopes
...
MCPishCode agent integration.

## Choosing a profile

Choose profiles from the compact subagent profile catalog returned by
`open_workspace`. Use the profile name with `mcpishcode agents run`. If no
...
```bash
mcpishcode agents run <profile-or-provider> --model <model> "<prompt>"
mcpishcode agents run <profile-or-provider> --thinking <level> "<prompt>"
```
...
the selected local agent harness; MCPishCode does not translate values between
providers.
...
MCPishCode tools unless the user asked for delegation, another agent's opinion,
