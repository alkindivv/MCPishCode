...
MCPishCode agent profiles are user-owned markdown files with YAML
frontmatter. They describe roles such as reviewer, explorer, or implementer.
MCPishCode owns provider invocation.
...
```bash
mcpishcode agents run <name> "<prompt>"
```

Use lowercase kebab-case names. If omitted, MCPishCode uses the filename without
`.md`.
...
Unsupported or custom providers are rejected. MCPishCode maps providers to their
native integration:
...
MCPishCode lets the provider default apply. Values are provider-specific
passthrough strings; MCPishCode does not translate names between harnesses.
...
MCPishCode passes this through to providers that expose a matching control:
...
The body is the profile prompt prefix MCPishCode prepends when launching that
...
mcpishcode agents run <profile-or-id> "<prompt>"
...
`mcpishcode agents ls` lists existing subagent sessions for the current workspace;
it does not list profile definitions.

The full profile body stays out of the model context until MCPishCode launches the
profile.
...
- First-class MCP agent tools. Future tools should wrap the same provider
  adapter registry used by `mcpishcode agents`.
