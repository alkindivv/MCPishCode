...
```bash
npx @alkindivv/mcpishcode config set publicBaseUrl https://your-tunnel-host.example.com
```
...
```bash
MCPISHCODE_PUBLIC_BASE_URL="https://new-tunnel.example.com" npx @alkindivv/mcpishcode serve
```
...
MCPishCode derives allowed hosts from the configured public URL.
...
```bash
MCPISHCODE_ALLOWED_HOSTS="*" npx @alkindivv/mcpishcode serve
```

## OAuth Redirect Host Rejected

By default, MCPishCode allows redirects for:
...
```bash
MCPISHCODE_OAUTH_ALLOWED_REDIRECT_HOSTS="chatgpt.com,example.com" npx @alkindivv/mcpishcode serve
```
...
```bash
npx @alkindivv/mcpishcode init --force
```
...
MCPishCode shell execution requires Bash. Native PowerShell and `cmd.exe` command
...
```bash
MCPISHCODE_SKILLS=1 npx @alkindivv/mcpishcode serve
```

MCPishCode looks in standard Agent Skills locations:
...
When `MCPISHCODE_SUBAGENTS=1`, MCPishCode loads agent profiles from
`~/.mcpishcode/agents/*.md` and project `.mcpishcode/agents/*.md`, then exposes a
...
`mcpishcode agents ls` lists existing subagent sessions, not profile
definitions.
