...
MCPishCode can save a file attached or generated by an MCP host, such as ChatGPT,
directly into an open workspace. Enable the tool with `MCPISHCODE_ARTIFACTS=1`.
...
3. Use the returned path with the ordinary MCPishCode filesystem tools.
...
MCPishCode creates missing parent directories and refuses to overwrite an existing
file. After the download, normal tools can inspect, move, rename, replace, or
delete it.

## Safety and limits

The `file` input must be the native value supplied by the MCP host. MCPishCode does
...
Downloads are streamed under `MCPISHCODE_ARTIFACT_MAX_FILE_BYTES` and published as
