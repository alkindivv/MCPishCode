...
const root = await mkdtemp(join(tmpdir(), "mcpishcode-oauth-test-"));
const oauthConfig = {
...
async function testDatabaseConfiguration(stateDir: string): Promise<void> {
  ...
  try {
    ...
    const migrations = database.sqlite
      .prepare("select version, name from mcpishcode_schema_migrations order by version")
      .all();
...
function testExpiredTokenCleanup(stateDir: string): void {
  ...
  store.saveTokenPair({
    ...
    accessToken: { clientId: client.client_id, scopes: ["mcpishcode"], expiresAt: expiredAt },
    refreshTokenHash: "expired-refresh-hash",
    refreshToken: { clientId: client.client_id, scopes: ["mcpishcode"], expiresAt: expiredAt },
  });
  store.close();
      ...
      store.saveTokenPair(
        {
          ...
          accessToken: { clientId: client.client_id, scopes: ["mcpishcode"], expiresAt },
          refreshTokenHash: "new-refresh-hash",
          refreshToken: { clientId: client.client_id, scopes: ["mcpishcode"], expiresAt },
        },
        "old-refresh-hash",
      ),
      true,
...
async function testProviderRestartRotationAndRevocation(stateDir: string): Promise<void> {
  ...
  try {
    ...
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, issued.refresh_token, ["mcpishcode"], mcpUrl),
      InvalidGrantError,
    );
    ...
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, refreshed.refresh_token, ["mcpishcode"], mcpUrl),
      InvalidGrantError,
    );
  } finally {
