...
const root = await mkdtemp(join(tmpdir(), "mcpishcode-workspace-test-"));
const outsideRoot = await mkdtemp(join(tmpdir(), "mcpishcode-workspace-outside-test-"));
...
try {
  ...
  await mkdir(join(root, ".mcpishcode", "agents"), { recursive: true });
  await writeFile(
    join(root, ".mcpishcode", "agents", "reviewer.md"),
    [
      "---",
      "name: reviewer",
  ...
  const config = loadConfig({
    MCPISHCODE_CONFIG_DIR: join(root, ".mcpishcode-home"),
    MCPISHCODE_ALLOWED_ROOTS: root,
    MCPISHCODE_WORKTREE_ROOT: join(root, ".mcpishcode", "worktrees"),
    MCPISHCODE_AGENT_DIR: agentDir,
  ...
  if (platform() !== "win32") {
    ...
    const unsafeConfig = loadConfig({
      MCPISHCODE_CONFIG_DIR: join(root, ".mcpishcode-unsafe-home"),
      MCPISHCODE_ALLOWED_ROOTS: root,
      MCPISHCODE_WORKTREE_ROOT: join(root, ".mcpishcode", "unsafe-worktrees"),
      MCPISHCODE_AGENT_DIR: unsafeAgentDir,
      MCPISHCODE_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
      PORT: "1",
    });
  ...
  await git(gitRoot, ["config", "user.email", "mcpishcode@example.com"]);
  await git(gitRoot, ["config", "user.name", "MCPishCode Test"]);
  ...
  if (platform() !== "win32") {
    ...
    const aliasConfig = loadConfig({
      MCPISHCODE_ALLOWED_ROOTS: aliasRoot,
      MCPISHCODE_WORKTREE_ROOT: join(aliasRoot, ".mcpishcode", "alias-worktrees"),
      MCPISHCODE_AGENT_DIR: agentDir,
      MCPISHCODE_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
      PORT: "1",
    });
