...
const root = await mkdtemp(join(tmpdir(), "mcpishcode-skills-test-"));
...
try {
  ...
  const mcpishcodeSkills = join(root, ".mcpishcode", "skills");
  ...
  await mkdir(join(mcpishcodeSkills, "mcpishcode-local-skill"), { recursive: true });
  ...
  await writeFile(
    join(mcpishcodeSkills, "mcpishcode-local-skill", "SKILL.md"),
    [
      "---",
      "name: mcpishcode-local-skill",
      "description: MCPishCode local skill description.",
      "---",
      "",
      "# MCPishCode Local Skill",
    ].join("\n"),
  );
  await writeFile(
  ...
  const disabledConfig = loadConfig({
    MCPISHCODE_ALLOWED_ROOTS: projectRoot,
    MCPISHCODE_AGENT_DIR: agentDir,
    MCPISHCODE_SKILL_PATHS: explicitSkills,
    MCPISHCODE_SKILLS: "0",
  ...
  const config = loadConfig({
    MCPISHCODE_ALLOWED_ROOTS: projectRoot,
    MCPISHCODE_AGENT_DIR: agentDir,
    MCPISHCODE_SKILL_PATHS: [explicitSkills, "~/.claude/skills", "./.claude/skills"].join(","),
    MCPISHCODE_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
    PORT: "1",
  });
  ...
  assert.equal(loaded.skills.some((skill) => skill.name === "mcpishcode-local-skill"), true);
  ...
  const duplicateConfig = loadConfig({
    MCPISHCODE_ALLOWED_ROOTS: projectRoot,
    MCPISHCODE_AGENT_DIR: agentDir,
    MCPISHCODE_SKILL_PATHS: [explicitSkills, "./.agents/skills"].join(","),
    MCPISHCODE_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
    PORT: "1",
  });
  assert.equal(
  ...
  const legacyPiConfig = loadConfig({
    MCPISHCODE_ALLOWED_ROOTS: projectRoot,
    MCPISHCODE_AGENT_DIR: agentDir,
    MCPISHCODE_SKILL_PATHS: [explicitSkills, join(projectRoot, ".pi", "skills")].join(","),
    MCPISHCODE_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
    PORT: "1",
  });
  assert.equal(
