...
{
  const mcpishcodeBin = `${process.cwd()}/node_modules/.bin`;
  const userBin = "/home/user/.local/bin";
  assert.equal(
    removeDevspaceNodeModulesBinFromPath([mcpishcodeBin, userBin].join(delimiter)),
    userBin,
  );

  const env = piCommandEnvironment({
    PATH: [mcpishcodeBin, userBin].join(delimiter),
  });
...

{
  ...
  const env = piCommandEnvironment({
    ...
    PATH: [mcpishcodeBin, "/home/user/.local/bin"].join(delimiter),
  });

  assert.equal(env.PATH, [mcpishcodeBin, "/home/user/.local/bin"].join(delimiter));
}
