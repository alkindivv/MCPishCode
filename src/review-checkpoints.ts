...
const REVIEW_REF_PREFIX = "refs/mcpishcode/review";
...
async function createWorkingTreeSnapshot(gitRoot: string): Promise<string> {
  const tempDir = await mkdtemp(join(tmpdir(), "mcpishcode-review-index-"));
  ...
  try {
    ...
    return (await git(gitRoot, ["commit-tree", tree, "-p", parent, "-m", "MCPishCode review snapshot"], { env })).stdout.trim();
  } finally {
...
function checkpointEnv(indexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "MCPishCode",
    GIT_AUTHOR_EMAIL: "mcpishcode@users.noreply.local",
    GIT_COMMITTER_NAME: "MCPishCode",
    GIT_COMMITTER_EMAIL: "mcpishcode@users.noreply.local",
  };
}
