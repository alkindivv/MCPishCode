...
const root = await mkdtemp(join(tmpdir(), "mcpishcode-artifact-download-test-"));
...
async function testCrashLeftoverCleanup(testRoot: string): Promise<void> {
  ...
  const stalePartial = join(destinationDirectory, ".mcpishcode-download-stale.partial");
  const recentPartial = join(destinationDirectory, ".mcpishcode-download-recent.partial");
  ...
  assert.equal(entries.includes(".mcpishcode-download-stale.partial"), false);
  assert.equal(entries.includes(".mcpishcode-download-recent.partial"), true);
