...
for (const flag of ["-v", "--version"]) {
  const output = execFileSync("node", ["--import", "tsx", "src/cli.ts", flag], {
    encoding: "utf8",
    env: { ...process.env, MCPISHCODE_CONFIG_DIR: "/tmp/mcpishcode-cli-version-test" },
  }).trim();

  assert.equal(output, packageJson.version);
}

const root = mkdtempSync(join(tmpdir(), "mcpishcode-cli-agents-test-"));
try {
  const configDir = join(root, ".mcpishcode");
  ...
  const output = execFileSync("node", ["--import", "tsx", "src/cli.ts", "agents", "ls"], {
    ...
    env: {
      ...
      MCPISHCODE_STATE_DIR: stateDir,
      MCPISHCODE_WORKSPACE_ID: "ws_current",
      MCPISHCODE_WORKSPACE_ROOT: projectRoot,
