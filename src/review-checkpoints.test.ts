...
const root = await mkdtemp(join(tmpdir(), "mcpishcode-review-checkpoints-test-"));

try {
  await git(root, ["init"]);
  await git(root, ["config", "user.email", "mcpishcode@example.com"]);
  await git(root, ["config", "user.name", "MCPishCode Test"]);
