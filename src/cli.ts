...
async function ensureConfigured(): Promise<void> {
  ...
  if (process.env.MCPISHCODE_OAUTH_OWNER_TOKEN) return;
  ...
  if (!input.isTTY || !output.isTTY) {
    throw new Error(
      [
        ...
        "  mcpishcode init",
        "",
        "Or provide MCPISHCODE_OAUTH_OWNER_TOKEN and MCPISHCODE_ALLOWED_ROOTS.",
      ].join("\n"),
    );
  }
...
async function runInit({ force }: { force: boolean }): Promise<void> {
  ...
  try {
    ...
    const portAnswer = await textPrompt({
      message: `Which local port should MCPishCode use? Press Enter to use ${defaultPort}`,
    ...
    prompts.note(
      [
        "MCPishCode needs a public base URL so ChatGPT or Claude can reach this MCP server.",
    ...
    prompts.note(lines.join("\n"), "MCPishCode configured");
    prompts.note(
      [
        `Owner password: ${auth.ownerToken}`,
        "Use this when ChatGPT or Claude asks you to approve MCPishCode access.",
        `Stored at: ${authPath}`,
      ].join("\n"),
      "Owner password",
    );
    prompts.outro("Run `mcpishcode serve` to start the MCP server.");
  } catch (error) {
...
async function serve(): Promise<void> {
  ...
  const httpServer = app.listen(config.port, config.host, () => {
    ...
    if (config.allowedHosts.includes("*")) {
      console.warn("warning: Host header allowlist is disabled because MCPISHCODE_ALLOWED_HOSTS=*");
    }
...
function printHelp(): void {
  console.log(
    [
      ...
      "  mcpishcode init            Create or update ~/.mcpishcode/config.json and auth.json",
      ...
      "  mcpishcode agents run <profile-or-provider-or-id> [--model <model>] <prompt>",
      "  mcpishcode agents show <id>",
      "  mcpishcode -v, --version   Print the installed version",
      "",
      "For temporary tunnels:",
      "  MCPISHCODE_PUBLIC_BASE_URL=https://example.trycloudflare.com mcpishcode serve",
    ].join("\n"),
  );
}
...
async function runAgentsRun(args: string[]): Promise<void> {
  ...
  const record = store.create({
    workspaceId: process.env.MCPISHCODE_WORKSPACE_ID,
    workspaceRoot,
...
async function runAgentsShow(args: string[]): Promise<void> {
  const [id] = args;
  if (!id) throw new Error("Usage: mcpishcode agents show <id>");
  ...
  if (record.status === "starting" || record.status === "running") {
    console.log(`No final response yet. Call \`mcpishcode agents show ${record.id}\` again later.`);
  }
}
...
async function runAgentsWorker(args: string[]): Promise<void> {
  ...
  if (!id || promptFileFlag !== "--prompt-file" || !promptFile) {
    throw new Error("Usage: mcpishcode agents __worker <id> --prompt-file <path>");
  }
...
function writeAgentPromptFile(prompt: string): string {
  const directory = mkdtempSync(join(tmpdir(), "mcpishcode-agent-prompt-"));
...
function resolveCurrentWorkspaceRoot(): string {
  return resolve(process.env.MCPISHCODE_WORKSPACE_ROOT || process.cwd());
}
...
function printAgentsHelp(): void {
  console.log(
    [
      ...
      "",
      "Usage:",
      "  mcpishcode agents ls",
      "  mcpishcode agents run <profile-or-provider-or-id> [--model <model>] [--thinking <level>] <prompt>",
      "  mcpishcode agents show <id>",
    ].join("\n"),
  );
}
